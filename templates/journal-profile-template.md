...
- `.claude/skills/review/SKILL.md` — entry point for `--peer [SHORT]`.

**Short-name uniqueness.** A short name is used as the `--peer` argument and must be unique across the file; `JHE` is Journal of Health Economics, the housing journal is `JHousE`.
