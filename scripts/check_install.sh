...
check_project() {
  ...
  want() {  # want <src-dir> <dest-subdir> [dirs-only]
    ...
    for item in "$src"/*; do
      ...
      name="$(basename "$item")"
      # Generated bytecode is not an upstream ITEM. Running a hook straight out of the
      # checkout writes hooks/__pycache__, which apply.sh never links and .gitignore already
      # excludes — so membership reported "1 upstream item never linked" in all six repos
      # for a directory that must never be linked. scripts/ was immune only because its
      # membership reads the SHIPPED manifest rather than globbing the directory. Same
      # exclusion the residue find at the bottom of this file already makes.
  ...
  # everywhere the moment it is seeded. protect-files.sh joined the seed 2026-09-24
  # (Phase 4.1, divergence D-3) -- the 2026-09-08 "opt-in per project" ruling (design
  # spec R-7, a different document's numbering than this repo's rulings appendix, which
  # has its own, unrelated R-7 -- the two were conflated in one 2026-09-23 audit note)
  # left every generated evidence file unprotected in all six repos, and `/review` had
  # already bolted on its own `git status --porcelain` check for exactly the failure this
