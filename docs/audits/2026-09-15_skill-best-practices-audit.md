...
| Med | ztp-setup* | Installs PyPI `zotpilot` while `VENDORED.md` names the fork. Stores API keys in `~/.config/zotpilot/config.json`, which contradicts the global `~/.secrets.env` rule. `new-project-ztp` promises micromamba + fork + `.mcp.json`, and ztp-setup does none of it. | Auditor |
