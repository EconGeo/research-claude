...
- Submission requires overall >= 95 AND every **scored** component >= 80. A 96 average with one component at 75 fails. An UNSCORED component does not fail the gate — it is not counted at all, so a paper with one component scored 100 and seven never scored reads 100 overall and PASSES. Check `state show` for coverage, not just `score` for a number.
