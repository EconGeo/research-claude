...
decision:** the vendored P1 offenders are **closed** — `EconGeo/ZotPilot#6` (ztp-tutor →
SKILL.md + `references/`, deploy ships them) and `EconGeo/ai-audit#3` (`/civilize` → steps;
...
`docs/improvement-ledger.md`, `scripts/check_render.py` as write-gate item 4). The evals for the
remaining skills closed the same day under `docs/plans/2026-09-25-remaining-skill-evals.md` (one
task per skill; `tests/evals/`); five live runs came back red on the skill, not the harness —
`/verify-claims` (vendored), `/revise`, `/submit`, `/talk`, `/strategize` — see that plan's
Findings. Those five skill fixes are the open work.
The earlier
