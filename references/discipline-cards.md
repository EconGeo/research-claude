...
## Real Estate and Urban Economics (`real-estate`)

**Paper-type frequencies (rough share of empirical work in top field journals).**

| Type | Share | Notes |
|---|---|---|
| Reduced-form | ~50% | DiD, IV, RDD, event study. Dominant mode for housing/urban papers. |
| Descriptive | ~20% | Market analysis, price index construction, hedonic estimation without causal claim. |
| Structural | ~15% | Spatial equilibrium models, search-and-matching (mortgage/housing), REIT asset pricing. |
...
- **Observational / archival:** preregistration uncommon; not expected at any field journal.
- **Replication packages:** JFQA requires deposit; REE/JREFE increasingly expect data and code availability on request; JUE follows general econ norms.

**Method conventions.**
- Significance stars: used across all field journals (unlike AEA flagship policy). Typical floor 10/5/1%.
- Standard-error reporting: clustered SEs at geographic unit standard (MSA, zip code, census tract); Conley spatial SEs required for cross-sectional geographic designs at JUE-tier; Newey-West for time-series asset pricing.
- Price/return measures: distinguish transaction-based (repeat-sales, hedonic) vs. appraisal-based (NCREIF NPI); appraisal smoothing must be addressed in return papers.
